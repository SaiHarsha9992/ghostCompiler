...
// Push job to Redis, now accepting an optional input field
app.post("/run", async (req, res) => {
    const { id, language, code, input } = req.body;
    if (!id || !language || !code) {
        return res.status(400).json({ error: "Missing id, language, or code" });
    }
    await redis.rPush("jobQueue", JSON.stringify({ id, language, code, input }));
